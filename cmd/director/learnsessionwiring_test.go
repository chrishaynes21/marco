...
// LearnSession, entered through the PRODUCTION request path.
//
...
// run. Learn has three call sites worth attacking — the request route, the pass adapter, and the
...
// TestLearnIsReachableThroughTheProductionRequestPath enters at Runtime.Observation, which is
// where the service delivers a decoded request. Deleting the `q.Learn != nil` branch must fail
// this.
func TestLearnIsReachableThroughTheProductionRequestPath(t *testing.T) {
	rt := &Runtime{observations: newObservationRegistry(), learn: &learnSession{}}

	// Nothing is being learned yet, and the read says so rather than inventing a session.
	if _, err := rt.Observation(service.ObserveQuery{
		Learn: &service.ObserveLearn{}}); err == nil {
		t.Fatal("reading a learn session before one exists returned a session")
	}
	...
	out, err := rt.Observation(service.ObserveQuery{
		Learn: &service.ObserveLearn{Name: "open downloads"}})
	if err == nil {
		t.Fatalf("Learn started with no window available: %+v", out)
	}
...
// TestLearningWithNoWindowNamedLearnsTheForegroundWindow is the whole point of not asking.
//
// A person saying "learn to open downloads" is looking at the thing they mean. Requiring
...
//
// Deleting the currentContext call in LearnSession must fail this.
func TestLearningWithNoWindowNamedLearnsTheForegroundWindow(t *testing.T) {
	...
	if _, err := rt.Observation(service.ObserveQuery{Learn: &service.ObserveLearn{
		...
		t.Fatalf("learning the window in front was refused: %v", err)
	}
	defer func() { _, _ = rt.learn.stop() }()

	rt.learn.mu.RLock()
	sel := rt.learn.selector
	rt.learn.mu.RUnlock()
...
// TestLearningRefusesWithoutDurableMemory holds the honest failure. A Director that cannot
// recognise anything cannot learn, and saying so is better than watching for a minute and
// then reporting that the starting place was not recognised.
func TestLearningRefusesWithoutDurableMemory(t *testing.T) {
	// NO memory: newObservationRegistry() with nothing installed.
	rt := &Runtime{observations: newObservationRegistry(), learn: &learnSession{}}
	_, err := rt.Observation(service.ObserveQuery{Learn: &service.ObserveLearn{
		Name: "open downloads", Target: windowref.Selector{EphemeralID: "window_1"},
	}})
	if err == nil {
		t.Fatal("Learn started against a Director with no durable memory")
	}
	if !strings.Contains(err.Error(), "cannot learn") {
		t.Errorf("the refusal was %q; it should say plainly that this Director cannot "+
			"learn", err)
	}
}

// TestLearningIsOneAtATime holds the same rule observation holds, for the same reason: two
// learn sessions would contend for the screen and neither could attribute what it saw.
func TestLearningIsOneAtATime(t *testing.T) {
	tc := &learnSession{}
	m := &stubTeachMemory{}
	...
	if _, err := tc.start("first", p, m, nil, nil,
		...
		t.Fatalf("starting the first learn session: %v", err)
	}
	waitFor(t, func() bool { return p.started() })

	if _, err := tc.start("second", p, m, nil, nil,
		windowref.Selector{}, "Downloads", "Open"); err == nil {
		t.Fatal("a second learn session started while the first was running")
	}
}

// TestCancellingLearningStopsEverything holds Part 28: no capture left running, nothing partial
// kept, and no pass started afterwards.
func TestCancellingLearningStopsEverything(t *testing.T) {
	tc := &learnSession{}
	m := &stubTeachMemory{}
	p := &stubTeachPasses{block: make(chan struct{})}
	...
	if s.Phase != learn.Cancelled {
		t.Fatalf("after stopping the phase is %q, want %q", s.Phase, learn.Cancelled)
	}
	if tc.running() {
		t.Error("the learn session is still running after being stopped")
	}
	...
	if m.pending() {
		t.Error("a pending demonstration request survived the cancel; an ordinary session " +
			"later would arm a capture for something nobody is learning")
	}
}
...
func TestAnUnusableNameNeverReachesTheDesktop(t *testing.T) {
	...
	if s.Phase != learn.Refused || s.Refusal != learn.NameNotUsable {
	...
	if tc.running() {
		t.Error("a refused learn session is still running")
	}
}

// ── the pass adapter ──────────────────────────────────────────────────────────

// TestALearnPassIsAnOrdinaryObservationSession holds the second call site. The adapter must go
// through the ordinary registry, with the selector it was given and the duration it was asked
// for; a pass that quietly widened either would be a learn session watching something the
// user did not choose.
func TestALearnPassIsAnOrdinaryObservationSession(t *testing.T) {
	g := newObservationRegistry()
	rt := &Runtime{observations: g}
	selector := windowref.Selector{EphemeralID: "window_7"}
	p := &learnPasses{rt: rt, selector: selector}
	...
	if err == nil && res.Session.State == observe.Completed {
		t.Fatal("a learn pass completed against a window that could not be acquired")
	}
}

// TestRunPassReturnsTheSessionsOwnRecord holds the registry addition. A caller that got the
// WRONG session's result would learn from evidence about something else entirely.
...
func waitFor(t *testing.T, cond func() bool) {
	...
	t.Fatal("timed out waiting for the learn session to reach its first pass")
}
...
func TestAPhraseThatCannotBecomeAPlayIsRefusedBeforeAnybodyDemonstratesAnything(t *testing.T) {
	...
	if err == nil {
		t.Fatal("a phrase that cannot become a play started a learn session")
	}
	...
	if s, ok := rt.learn.read(); ok {
		t.Fatalf("a learn session was created for an unusable name: %+v", s)
	}
}
...
func TestTheLearnViewReportsTheArtifactAndTheQuestion(t *testing.T) {
	// A session that reached completion with nothing written must not claim it learned.
	unsaved := viewLearn(learn.Session{Name: "open downloads", Phase: learn.Complete}, false, false)
	if unsaved.Learned {
	...
	saved := viewLearn(learn.Session{
		Name: "open downloads", Phase: learn.Complete,
		Saved: &learn.Saved{Name: "downloads-open", Saved: true},
	}, false, false)
	...
	asking := viewLearn(learn.Session{
		Phase:     learn.Naming,
		SessionID: "observe_3",
		Question:  &learn.Question{ID: "q_name", SessionID: "observe_3"},
	}, true, false)
...
func TestTheViewSaysWhatThePlayWillBeCalledBeforeAnythingIsDemonstrated(t *testing.T) {
	v := viewLearn(learn.Session{
		Name: "open mouse settings", Phase: learn.EstablishingStart,
	...
	if v.Phase != learn.EstablishingStart {
		t.Fatalf("fixture phase drifted to %q", v.Phase)
	}
	// A session with no names yet claims none.
	if bare := viewLearn(learn.Session{Name: "open downloads"}, true, false); bare.WillBeCalled != "" {
