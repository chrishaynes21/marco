...
// Which window a teach session decides it is about.
//
...
func settling(t *testing.T, seq []uint32) uint32 {
	t.Helper()
	at, last := 0, uint32(0)
	p := &teachPasses{rt: &Runtime{winDirectory: windowref.NewDirectory()}}
...
// A window merely PASSED THROUGH is not the one being taught.
//
...
func TestTheSettleRuleComparesTheWindow(t *testing.T) {
	...
	p := &teachPasses{rt: &Runtime{winDirectory: dir}}
	p.subject = func(context.Context) (windowref.Candidate, error) {
		polls++
...
func TestANamedWindowSkipsTheWait(t *testing.T) {
	p := &teachPasses{selector: windowref.Selector{ProcessID: 99}}
...
// without it the panel is what gets taught about. The platform chokepoint excludes Marco's
...
func TestTheSettleRuleNeverLatchesMarcosOwnSurface(t *testing.T) {
	...
	if _, err := rt.foregroundCandidate(context.Background()); err == nil {
		t.Fatal("Marco's own surface was offered as the window to teach about.\nPressing " +
