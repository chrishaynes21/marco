...
// The Learn side of pointing, entered where production enters it.
//
...
// production start never installed one and no learn session could ever point at anything.

// TestLearningInstallsGroundingThroughTheProductionStart enters at learnSession.start.
//
// Deleting `.WithGrounding(ground)` must fail this.
func TestLearningInstallsGroundingThroughTheProductionStart(t *testing.T) {
	tc := &learnSession{}
	g := &countingGround{}
	p := &oneTeachPass{}
...
// TestAGroundedLearnViewCarriesDesktopRectangles holds the conversion.
//
...
// taken with — and this is the test that the learn path actually goes through it.
func TestAGroundedLearnViewCarriesDesktopRectangles(t *testing.T) {
	...
	rt.learnGround = newLearnGrounding(rt)
	rt.learnGround.frames[observe.ReferentLearnStart] = sampledFrame{
	...
	s := learn.Session{
		...
		StartReferent: &observe.VisualReferent{
			Role: observe.ReferentLearnStart, About: "2 controls I recognise this screen by",
