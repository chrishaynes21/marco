...
// `director learn "open downloads"` — the human-facing way in.
//
// It starts a learn session in the service and then follows it, printing each new thing Marco
// has to say. Following rather than driving: every decision is made in the service, and this
// process could be killed and restarted without changing what is being learned.
//
...
// learnPollInterval is how often the follower asks what changed.
//
...
const learnPollInterval = 500 * time.Millisecond
...
func runLearn(args []string) int {
	...
	// and a learn session named after a window id is a confusing way to lose ten minutes.
	name, rest := "", args
	...
	windowID := fs.String("window-id", "", "learn against the window with this ephemeral id")
	title := fs.String("window-title", "", "learn against the window whose title contains this")
	application := fs.String("application", "", "learn against this application's window")
	processID := fs.Int("process", 0, "learn against this process's window")
		...
		if !q.Target.Zero() {
			if err := q.Target.Validate(); err != nil {
				fmt.Fprintf(os.Stderr,
					"director: %v\n\nName one window, or none at all to learn against "+
						"whatever you're working in:\n"+
						"  director learn %q\n"+
						"  director learn %q --window-id window_37   (director windows "+
						"lists them)\n", err, name, name)
				return 2
			}
		}
	...
	view, err := learnRequest(client, q)
	if err != nil {
	...
	// the end of Learn — the user answers it in another terminal, the coordinator notices,
	// and the story continues in this one.
	for {
		time.Sleep(learnPollInterval)
		view, err = learnRequest(client, service.ObserveLearn{Evidence: *watch})
		if err != nil {
			fmt.Fprintf(os.Stderr, "director: %v\n", err)
			return 1
		}
		printed = printLearn(view, *watch, printed)
		shown.print(view)
		printLearnFooter(view, &asked)
		if view.Settled {
			return 0
...
// learnRequest sends one learn request and decodes the view.
func learnRequest(client *service.Client, q service.ObserveLearn) (learnSessionView, error) {
	raw, err := client.Observation(service.ObserveQuery{Learn: &q})
	if err != nil {
		return learnSessionView{}, err
	}
	var v learnSessionView
...
// printLearn prints what changed since the last reading, and returns the new one.
//
...
func printLearn(v learnSessionView, watch bool, last string) string {
	if watch {
...
func launchHighlight(e groundedView, hold time.Duration) (*exec.Cmd, error) {
	...
	// Reaped in the background so a long learn session does not accumulate zombies.
	go func() { _ = cmd.Wait() }()
	return cmd, nil
}

func (g *groundingShown) print(v learnSessionView) {
	if g.seen == nil {
...
// printLearnFooter says what to do next, when there is something to do.
//
// Marco asks; the person answers through the commands that already accept answers. Learn
// creates no shortcut around them, because two of the three questions below are about giving
// Marco permission or giving it a word, and neither is Learn's to supply.
// printLearnFooter says what to do next, once per question.
//
...
func printLearnFooter(v learnSessionView, asked *string) {
	...
	case learn.Refused:
		*asked = key
		// "NOTHING WAS SAVED" IS NOT ALWAYS TRUE. A learn that wrote the play and could not
		...
		if v.Play != "" {
			...
			fmt.Println("Nothing can ask for it yet. `director learn --watch` says why; " +
		...
		fmt.Printf("\nNothing was saved. `director learn --watch` shows why, or try again:\n"+
			"  director learn %q\n", v.Name)
		if v.Refused == learn.DestinationNotRecognised {
