...
// The Learn tail, against the REAL lifecycle.
//
...
func taughtTail(t *testing.T, g *observationRegistry) (*learnTail, observe.RelationshipRef) {
	t.Helper()
	grant := g.last.Grant()
	if grant == nil {
	...
	return &learnTail{rt: rt, app: func() string { return "testgame" }}, grant.Relationship
}

// ── the tail reaches a play on disk ───────────────────────────────────────────

func TestTheLearnTailWritesTheRealPlayThroughTheRealSavePath(t *testing.T) {
	dir := t.TempDir()
	...
	//
	// Learn used to ask only to save, and Marco.s completion sentence — "you can ask me to
...
// Part 15/28 — the artifact survives a restart and the learn session does not.
func TestAfterARestartThePlaySurvivesAndTheLearnSessionDoesNot(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("MARCO_ROUTES", dir)
	...
	// A FRESH everything: new runtime state, new learnSession holder, new semantic store opened
	// from the same path. Nothing is carried over in memory.
	fresh := &Runtime{observations: newObservationRegistry(), learn: &learnSession{}}
	if s, ok := fresh.learn.read(); ok {
		t.Fatalf("a learn session survived the restart: %+v", s)
	}
	if _, err := fresh.LearnSession(context.Background(), teachRead()); err == nil {
		t.Error("reading learn state after a restart returned a session")
	}
...
// Part 16 — completing a learn runs nothing.
//
...
func TestTheLearnTailNeverInvokesWhatItJustSaved(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("MARCO_ROUTES", dir)
	...
	for _, forbidden := range []string{"Run", "Execute", "Invoke", "Perform", "Do", "Play"} {
		if _, has := v.MethodByName(forbidden); has {
			t.Errorf("routes.Route has a %s method; Learn would then be performing",
				forbidden)
		}
	}
}
...
func TestTheTailSurfacesTheOrdinaryNamingQuestion(t *testing.T) {
	...
	tail := &learnTail{rt: rt, app: func() string { return "testgame" }}
	...
	if _, ok := tail.Question(grant.Relationship, observe.AskNameScreen); !ok {
		t.Error("lowering demanded a name and no naming question was raised in the ledger; " +
			"Learn cannot invent one and must not")
	}
	// And Learn has no way to answer it itself: the Tail interface has no naming method.
	assertNoNamingMethod(t, tail)
}

// ── helpers ───────────────────────────────────────────────────────────────────

func teachRead() service.ObserveLearn { return service.ObserveLearn{} }
...
// assertNoNamingMethod holds the structural half of "Learn never names a screen".
func assertNoNamingMethod(t *testing.T, tail learn.Tail) {
	t.Helper()
	if _, ok := tail.(interface {
		NameSubject(string, string, observe.ScreenName) error
	}); ok {
		t.Error("the learn tail can name a subject directly; naming must go through the " +
...
// would let a learn session spend a permission the user gave about a different journey.
...
//
// Learn waits on a question until it is answered. A tail that kept handing back a settled
...
// learn two flags.
//
// The objection is answered rather than overruled: the derivation is SAID OUT LOUD (learnSessionView
...
func TestALearnedPlayIsRegisteredWhenItIsSaved(t *testing.T) {
	...
	//
	// — Marco offering to learn a play it had learned four minutes earlier. A context route
...
// # The live failure
//
// The Audience asked Marco to learn "Open Mouse Settings". The play's Marco identity is
// `MouseSettings's Open`, and the route slug was taken from the actor — so the route
// registered as `mousesettings` and their own words produced only:
//
...
func TestTheTaughtPhraseIsWhatResolves(t *testing.T) {
	...
	if saved.Name != want {
		t.Errorf("the play registered as %q; the Audience called it \"Open Mouse Settings\" and "+
	...
	if !found {
		t.Errorf("route discovery does not list %q, so the phrase they asked for resolves to nothing",
			want)
	}
}
