...
// The rest of the lifecycle, as the teaching coordinator reaches it.
//
...
type teachTail struct {
	rt *Runtime
	// app is which application is being taught, read LATE.
	//
	// A function rather than a string because the answer is not known when teaching starts:
	...
	//
	// It is what they will ask for, and it is not the play.s Marco identity: a person who
	// taught "Open Mouse Settings" gets `do MouseSettings.s Open` in the file and expects
...
var (
	...
	_ teach.GrantDiagnoser    = (*teachTail)(nil)
	_ teach.QuestionDiagnoser = (*teachTail)(nil)
)
...
func (t *teachTail) Question(route observe.RelationshipRef, kind observe.AskKind) (
	teach.Question, bool) {
	...
	for i := len(g.finished) - 1; i >= 0; i-- {
		...
		for _, p := range res.Proposals.Proposals {
			...
			q := teach.Question{ID: p.ID, SessionID: res.Session.ID}
			if p.Screen != nil {
...
func (t *teachTail) Granted(route observe.RelationshipRef) bool {
	g := t.rt.observations
	if g == nil {
		return false
	}
...
// optional half of Granted, probed by the coordinator through teach.GrantDiagnoser.
//
// It exists because every cause used to be a silent return: the person answered yes, no
// grant appeared, and ten minutes later teaching reported `rehearsal_declined` as though
// nobody had answered at all. The runner now records the closed reason; this carries it up.
func (t *teachTail) GrantRefusal(route observe.RelationshipRef) string {
	g := t.rt.observations
	if g == nil {
		return ""
	}
...
func (t *teachTail) Rehearse(context.Context) (teach.Attempt, error) {
	v, err := t.rt.Rehearse(service.ObserveRehearse{Live: t.live})
	if err != nil {
		return teach.Attempt{}, err
	}
	return teach.Attempt{
...
// that on every read and has since the naming lifecycle landed — Teach does not ask for it and
// could not.
func (t *teachTail) Lowering(route observe.RelationshipRef) (teach.Readiness, error) {
	v, err := t.rt.LearnedPlay(service.LearnedQuery{Application: t.app()})
	if err != nil {
		return teach.Readiness{}, err
	}
	for _, p := range v.Plays {
		if p.From != route.From || p.To != route.To {
			continue
		}
		r := teach.Readiness{Eligible: p.Eligible, Refusals: p.Refusals, Source: p.Source}
		if len(p.Unnamed) > 0 {
			r.Unnamed = p.Unnamed[0]
		}
		return r, nil
	}
	return teach.Readiness{}, fmt.Errorf(
...
func attemptSteps(in []service.RehearsalStepView) []teach.AttemptStep {
	out := make([]teach.AttemptStep, 0, len(in))
	for _, s := range in {
		out = append(out, teach.AttemptStep{
...
func (t *teachTail) QuestionRefusal(route observe.RelationshipRef, kind observe.AskKind) string {
	g := t.rt.observations
	if g == nil {
		return ""
	}
...
func (t *teachTail) OfferRehearsal(route observe.RelationshipRef) error {
	g := t.rt.observations
	if g == nil {
...
func (t *teachTail) AnswerToRehearsal(route observe.RelationshipRef) (observe.UserResponse, bool) {
	g := t.rt.observations
	if g == nil {
...
func (t *teachTail) SaveRoute(walk []observe.RelationshipRef, actor, verb string) (
	teach.Saved, error) {
...
func (t *teachTail) taught() string {
	if t.phrase == nil {
		return ""
	}
	return t.phrase()
