...
// Showing a person the two decisions a teach session makes before it asks them for anything.
//
// # What is grounded
//
...
// state. Nothing is written anywhere and a restart loses it, which is correct: a teach session
// does not survive a restart either.

// teachGrounding answers "where is that screen" for the teaching coordinator.
//
// It decides nothing about teaching. It converts a screen the coordinator has already settled on
// into the regions it currently occupies, through the same resolver every other referent uses.
type teachGrounding struct {
	rt *Runtime
...
var _ teach.Grounding = (*teachGrounding)(nil)

func newTeachGrounding(rt *Runtime) *teachGrounding {
	return &teachGrounding{rt: rt, frames: map[observe.ReferentRole]sampledFrame{}}
}

// Ground resolves one pinned screen to where it currently is.
func (g *teachGrounding) Ground(t observe.ShadowTotals, application string,
...
func (g *teachGrounding) frameFor(role observe.ReferentRole) (sampledFrame, bool) {
	g.mu.Lock()
	defer g.mu.Unlock()
	f, ok := g.frames[role]
	return f, ok
...
func (r *Runtime) groundingViews(ctx context.Context, s teach.Session,
