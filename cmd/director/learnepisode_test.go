...
//
//  1. Can one explicit learn attempt satisfy a policy that exists to mean independent real-world
//     recurrence?
//  2. Does history at the starting screen make direct Learn unusable?
//
...
// is what catches it — this exists to make the Learn side's contribution legible.
...
// TestALearningEpisodeCorroboratesOnce is the answer to the audit's question.
//
// Three bounded passes in one sitting are one learn episode. The evidence folds every time —
...
func TestALearningEpisodeCorroboratesOnce(t *testing.T) {
	route := observe.RelationshipRef{From: "subj_start", To: "subj_end"}
	store := newEpisodeStore()

	// A learn attempt's three passes, replayed through the same stamping the runner does.
	passes := &learnPasses{}
	for i := 0; i < 3; i++ {
		...
		// What learnPasses.Observe does with the result.
	...
	if got := store.sessions[route]; got != 1 {
		t.Fatalf("one learn attempt claimed %d independent sessions, want 1.\n"+
			"Three bounded passes in one sitting are one episode; counting each of them "+
			"would let an explicit learn manufacture the corroboration that the invitation "+
...
// get wrong: only Learn says otherwise, and it says so through RunPass.
...
// TestHistoryAtTheStartDoesNotMakeLearningUnusable characterises the branching rule.
//
// Learn picks the route by DIFFING the durable topology across the discovery pass. What decides
...
// direct Learn unusable, and the counters say it does not.
func TestHistoryAtTheStartDoesNotMakeLearningUnusable(t *testing.T) {
	start := "subj_start"
...
//
// Not restated here — `learnPasses.Observe` is called, and what is asserted is the flag it hands
// the session runner. A test that reimplemented the rule would go on passing after somebody
// deleted it.
func TestALearnEpisodeClaimsOneCorroborationThroughTheProductionPass(t *testing.T) {
	var asked []bool
