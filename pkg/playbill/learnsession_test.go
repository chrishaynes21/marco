...
// Three readings of one learn session, and they must describe the same truth.
//
...
func learnSession(t playbill.LearnSession) playbill.View {
	v := playbill.View{Reach: playbill.Present, LearnSession: t}
	return v.Normalise()
}
...
func TestBeingWatchedNormallyAndBeingShownAreUnmistakablyDifferent(t *testing.T) {
	ready := learnSession(playbill.LearnSession{Active: true, Asked: "open target"})
	armed := learnSession(playbill.LearnSession{Active: true, Asked: "open target", Armed: true})
...
func TestTheTwoSilencesAboutWhatYouDidAreKeptApart(t *testing.T) {
	nothing := learnSession(playbill.LearnSession{Active: true, Asked: "open target"})
	unknown := learnSession(playbill.LearnSession{
		Active: true, Asked: "open target", Unattributed: true})
	known := learnSession(playbill.LearnSession{
...
func TestOnlyNavigationMeaningsMayBeShownAsWhatYouDid(t *testing.T) {
	for _, bad := range []string{"VK_RETURN", "Enter", "a", "ctrl+s", "password"} {
		v := playbill.View{Reach: playbill.Present,
			LearnSession: playbill.LearnSession{Active: true, Did: []string{bad}}}
	...
	ok := playbill.View{Reach: playbill.Present,
		LearnSession: playbill.LearnSession{Active: true, Did: []string{"down", "confirm", "back"}}}
...
func TestWhatYouDidIsBoundedRatherThanATranscript(t *testing.T) {
	...
	v := playbill.View{Reach: playbill.Present,
		LearnSession: playbill.LearnSession{Active: true, Did: long}}
	if err := v.Normalise().Admit(); err == nil {
...
func TestNothingClaimsAPlayWasLearnedWithoutOne(t *testing.T) {
	...
	stopped := learnSession(playbill.LearnSession{
	...
	learned := learnSession(playbill.LearnSession{
	...
	lying := playbill.View{Reach: playbill.Present,
		LearnSession: playbill.LearnSession{Active: true, Learned: ""}}
	lying.LearnSession.Registered = true
...
func TestTheChecklistIsRenderedFromWhateverStateItIsGiven(t *testing.T) {
	v := learnSession(playbill.LearnSession{Active: true, Asked: "open target", Progress: []playbill.Step{
...
func TestTheLearningSectionNeverNamesDirectorsBackstage(t *testing.T) {
	v := learnSession(playbill.LearnSession{
	...
	for _, forbidden := range []string{"subj_", "state_", "fingerprint", "digest",
		...
		if strings.Contains(strings.ToLower(text), forbidden) {
			t.Errorf("the Learn surface leaks %q:\n%s", forbidden, text)
		}
	}
}
...
func TestAPlayNameTravelsAndAPathDoesNot(t *testing.T) {
	v := playbill.View{Reach: playbill.Present,
		LearnSession: playbill.LearnSession{Learned: `C:\Users\someone\routes\open-target.marco`}}
	if err := v.Normalise().Admit(); err == nil {
		t.Fatal("a file path was admitted as a learned play name")
	}
	ok := playbill.View{Reach: playbill.Present,
		LearnSession: playbill.LearnSession{Learned: "open target"}}
	if err := ok.Normalise().Admit(); err != nil {
...
func TestTheCueMovesTheDigestSoNoSurfaceCanSleepThroughIt(t *testing.T) {
	before := learnSession(playbill.LearnSession{Active: true, Asked: "open target"}).WithDigest()
	after := learnSession(playbill.LearnSession{
	...
	did := learnSession(playbill.LearnSession{
...
// An idle Director says nothing about Learn at all.
func TestNoLearningSessionShowsNoLearningSection(t *testing.T) {
	...
	// present on its own is a producer bug, and rendering a bare LEARN SESSION heading for it
	// would tell somebody Marco is learning when it is not.
	if w := watchText(learnSession(playbill.LearnSession{
		Because: "Hold still a moment.", Examples: 2,
		Progress: []playbill.Step{{Name: "Show me", State: playbill.StepCurrent}},
	})); strings.Contains(w, "LEARN SESSION") {
		t.Errorf("a leftover sentence raised a Learn section:\n%s", w)
	}

	v := learnSession(playbill.LearnSession{})
	if w := watchText(v); strings.Contains(w, "LEARN SESSION") {
		t.Errorf("an idle Director shows a Learn section:\n%s", w)
	}
	if h := v.Normal(); h.Word == "Learning from you" || h.Word == "Show me" {
...
//
// `LearnSession.Stage` derives Saved from `Learned` being set, so the only way to reach that
...
func TestAnAccountCannotClaimAPlayItCannotName(t *testing.T) {
	...
	for _, ok := range []playbill.LearnSession{
		{},
		...
		v := playbill.View{Reach: playbill.Present, LearnSession: ok}
		if err := v.Normalise().Admit(); err != nil {
	...
	if (playbill.LearnSession{Active: true}).Stage() == playbill.Saved {
		t.Error("an active session with nothing written reads as saved")
	}
	if (playbill.LearnSession{Learned: "open target"}).Stage() != playbill.Saved {
		t.Error("a written play does not read as saved")
	}
	if (playbill.LearnSession{Active: true, Armed: true}).Stage() != playbill.ShowMe {
		t.Error("an armed session does not read as show-me")
	}
	// Stopped outranks active: an attempt that ended is not still going.
	if (playbill.LearnSession{Active: true, Stopped: true}).Stage() != playbill.NotLearning {
...
// identically as "Learning from you". A surface whose reassurance outlives its truth is
// worse than one that says nothing.
func TestWaitingOnAPersonIsItsOwnReadingAndReassuranceDoesNotGoStale(t *testing.T) {
	thinking := learnSession(playbill.LearnSession{Active: true, Asked: "open target",
		Because: "Thinking about what I saw.",
		Progress: []playbill.Step{
			{Name: "Starting place", State: playbill.StepDone},
			{Name: "Show me", State: playbill.StepCurrent}}})
	waiting := learnSession(playbill.LearnSession{Active: true, Waiting: true, Asked: "open target",
	...
	start := learnSession(playbill.LearnSession{Active: true, Asked: "open target",
