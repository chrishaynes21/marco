...
func (b *Buffer) Moved(application, from, to string, by Source, at time.Time) {
	...
	key := edgeKey{from: from, to: to, application: application}
	e, ok := b.edges[key]
	if !ok {
		b.evictEdgeLocked()
		e = &Edge{From: from, To: to, Application: application,
			By: map[Source]int{}, First: at}
		b.edges[key] = e
	}
	e.Seen++
	e.By[by]++
	...
	b.moves = append(b.moves, Move{
		From: from, To: to, Application: application, By: by, At: at,
	})
