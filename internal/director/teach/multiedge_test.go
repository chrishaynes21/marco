...
// one. Teaching gets a single exempt rehearsal question, the terminal leg claimed it, and when
...
func (w *walkTail) Rehearse(context.Context) (teach.Attempt, error) {
	w.rehearsed = append(w.rehearsed, w.asked)
	if why, bad := w.refuse[w.asked]; bad {
		return teach.Attempt{Attempted: false, Refusal: why}, nil
	}
...
func demonstrating(t *testing.T, route []observe.RelationshipRef, granted bool,
	refuse map[observe.RelationshipRef]string) (*teach.Coordinator, *walkTail, *stubTail) {
	...
	c := teach.New("open mouse settings", p, m,
...
func walked(t *testing.T, route []observe.RelationshipRef,
	refuse map[observe.RelationshipRef]string) (teach.Session, *walkTail) {
...
func TestBothDemonstratedEdgesAreOffered(t *testing.T) {
	...
	for i, e := range s.Edges {
		if e.Status != teach.EdgeVerified {
			t.Errorf("edge %d (%v) is %s, want verified", i+1, e.Route, e.Status)
		}
	}
	if got := s.Status(); got != teach.RouteVerified {
...
func TestAThreeEdgeDemonstrationReviewsAllThree(t *testing.T) {
	...
	if s.Status() != teach.RouteVerified {
...
func TestARefusedFirstEdgeDoesNotEndTheReview(t *testing.T) {
	...
	if s.Status() != teach.RoutePartial {
...
//
// It models the real thing: teaching's single exempt slot has already gone to the terminal leg,
...
func (o *offeringTail) Question(r observe.RelationshipRef, kind observe.AskKind) (
	...
	if kind == observe.AskRehearse {
		...
		if !o.open[r] || o.answer[r] != observe.ResponseNone {
			return teach.Question{}, false
		}
		return teach.Question{ID: "q_rehearse", SessionID: "observe_3"}, true
	}
...
func (o *offeringTail) Rehearse(context.Context) (teach.Attempt, error) {
...
func offering(t *testing.T, route []observe.RelationshipRef,
	decline map[observe.RelationshipRef]bool) (teach.Session, *offeringTail) {
...
func offeringTuned(t *testing.T, route []observe.RelationshipRef,
	decline map[observe.RelationshipRef]bool, wrap func(*offeringTail) teach.Tail) (
	teach.Session, *offeringTail) {
...
func TestABusyQuestionSlotDoesNotWriteOffTheStep(t *testing.T) {
	route := []observe.RelationshipRef{edge(placeA, placeB), edge(placeB, placeC)}
	s, o := offeringTuned(t, route, nil, func(o *offeringTail) teach.Tail {
	...
	if got := s.Edges[0].Status; got == teach.EdgeUnresolved {
...
func TestAYesIsNotReadAsARefusalBeforeTheGrantExists(t *testing.T) {
	...
	if got := s.Edges[0].Status; got == teach.EdgeUnresolved || got == teach.EdgeDeclined {
...
func TestARefusedStepSaysWhichRefusalItWas(t *testing.T) {
	...
	if got := s.Edges[0].Status; got != teach.EdgeDeclined {
...
//
// A teach episode runs bounded observation passes back to back, and each one brings a new Runner
...
func TestARetractedQuestionEndsTheLeg(t *testing.T) {
	route := []observe.RelationshipRef{edge(placeA, placeB), edge(placeB, placeC)}
	s, _ := offeringTuned(t, route, nil, func(o *offeringTail) teach.Tail {
	...
	if got := s.Edges[0].Status; got == teach.EdgeOffered || got == teach.EdgePending {
...
func TestAYesIsNeverReportedAsADecline(t *testing.T) {
	...
	if s.Refusal == teach.RehearsalDeclined {
	...
	if s.Refusal != teach.RehearsalNotStarted {
		t.Errorf("the outcome is %q, want %q", s.Refusal, teach.RehearsalNotStarted)
	}
}
...
func (v *savingTail) SaveRoute(walk []observe.RelationshipRef, actor, verb string) (
	...
	return teach.Saved{Name: "mousesettings", Saved: true}, nil
}

func (v *savingTail) Save(r observe.RelationshipRef, actor, verb string) (teach.Saved, error) {
	v.edge = r
...
func TestAPartialRouteDoesNotSaveAsAWholeOne(t *testing.T) {
	...
	offeringTuned(t, route, map[observe.RelationshipRef]bool{route[0]: true},
		func(o *offeringTail) teach.Tail {
			saver = &savingTail{offeringTail: o}
			return saver
		})
	if saver.route {
