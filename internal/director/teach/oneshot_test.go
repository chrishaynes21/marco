...
//
// Every teach test in this package built `CandidateAssessment{Verdict: CandidateConsistent}` with
...
func TestOneCleanDemonstrationOffersToTryRatherThanAskingAgain(t *testing.T) {
	c, _, p := teachToDemonstration(t, goodCandidate(), realFirstAssessment())
...
// Teach READS the candidate the session produced and asks for nothing more.
//
...
// grant it waits for cannot exist. Teach's job is to notice the demonstration and move on.
//
...
func TestTheDiscoveryPassBecomesTheDemonstrationWithoutAskingAgain(t *testing.T) {
	...
	if s.Phase != teach.ReadyToRehearse {
		t.Fatalf("after ONE performance the phase is %q, want %q\n%s",
			s.Phase, teach.ReadyToRehearse, strings.Join(s.Watch(), "\n"))
	}
	if s.Examples != 1 {
		t.Errorf("examples = %d, want 1", s.Examples)
	}
...
func TestWhenTheWatchedPassCannotSupportACandidateTheCaptureStillRuns(t *testing.T) {
	...
	if s.Phase != teach.NeedsAnotherExample {
		t.Fatalf("phase is %q, want %q — with nothing attributed the armed capture has to "+
			"take over", s.Phase, teach.NeedsAnotherExample)
	}
	if s.Examples != 0 {
...
func TestWithoutASuccessfulRehearsalOneDemonstrationLearnsNothing(t *testing.T) {
	for _, tc := range []struct {
		...
		answer  func(*stubTail, teach.Phase)
	}{{
		name: "permission was never given",
		answer: func(tail *stubTail, p teach.Phase) {
			if p == teach.Naming {
				tail.name()
			}
		},
...
func TestOneShotDoesNotMeanAcceptAnything(t *testing.T) {
	for _, tc := range []struct {
		...
		verdict: observe.CandidateAmbiguous, want: teach.NeedsAnotherExample,
	}, {
		name: "a screen on the route with no identity",
		reasons: []observe.AssessmentReason{
			observe.ReasonSingleDemonstration, observe.ReasonTransientCheckpoint,
		},
		verdict: observe.CandidateInsufficient, want: teach.NeedsAnotherExample,
	}, {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if s.Phase == teach.NeedsAnotherExample && len(s.Uncertain) == 0 {
