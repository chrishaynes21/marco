// Package learn is the human-facing orchestration of Marco's existing learning chain.
//
// # What it is
//
...
//
// It exists because that machinery is complete and unreachable: a person who wants Marco to
// learn something has, until now, had to wait for Marco to notice a habit, offer to learn it, and
// then be shown. Learn reverses the timing without touching the evidence model.
//
...
// assessment, rehearsal and the wrong-destination guard all rest on — Learn lets the FIRST pass
...
// user's "yes" to "shall I learn this?" writes today, and `learn "..."` IS that yes — given in
...
//
// # What Learn may not do
//
//   - It creates no authority. Learn is permission to observe a bounded session, nothing more.
...
//     the package boundary test holds — so "Marco demonstrated it to itself" is not
//     reachable by mistake. The only string it holds that came from a person is the name they asked
...
// proposal — and the phase only says which of them Learn is currently waiting on.
type Phase string
...
const (
	...
	// learned in one sitting — the person had to demonstrate the first leg again, separately.
	//
	...
	// Complete, Refused and Cancelled are the three ways a learn session ends.
	Complete  Phase = "complete"
	Refused   Phase = "refused"
	Cancelled Phase = "cancelled"
)

// Settled reports whether the learn session is over.
func (p Phase) Settled() bool {
	return p == Complete || p == Refused || p == Cancelled
}

// Waiting reports whether Learn is blocked on the user answering an existing question.
//
...
// Refusal is the CLOSED vocabulary of why a learn session stopped.
//
// Discrete, like every other refusal in this repository. There is no learning score: either
...
const (
	...
	// Learn does not pick a winner and does not average them.
	...
	// ExamplesExhausted is the bound on how many times Learn will ask for one more.
...
// MaxExamples bounds how many demonstrations one learn session will ask for.
//
// Two. Existing policy wants a second example to corroborate the first; a third is a session that
// has stopped converging, and asking indefinitely is how a learning flow becomes a chore.
const MaxExamples = 2
...
type Bounds struct {
	...
	//
	// A learn session is ephemeral and holds a window under observation; one left open
...
// Passes runs one bounded observation pass against the window Learn was started for.
//
// The ENTIRE perception and session chain sits behind this. Learn cannot start a session of its
...
// Grounding turns a screen Learn has just decided about into somewhere on the display.
//
...
//
// So Learn says which screen it means and gets back where that currently is, in the ordinary
...
//
// There is nothing for a learn session to DO about not being able to point. The referent carries
...
// Session is what a learn attempt looks like from outside.
//
...
type Session struct {
	...
	//
	// An ADDRESS, not the question: the proposal owns its own wording and Learn does not
	// paraphrase it.
	Question *Question
...
// Coordinator drives one learn attempt.
//
...
// New starts a learn attempt for a name the user supplied.
//
...
// cannot point can still learn perfectly well; it simply cannot show the user what it decided, and
...
// save time. A learn attempt that fails must not leave the user's word attached to anything.
...
func (c *Coordinator) Cancel() {
	...
	// route nobody is learning any more.
	c.withdraw()
}
...
func (c *Coordinator) Advance(ctx context.Context) Session {
	...
	case WaitingForStart:
		...
		// authority rather than from a timer of Learn's own.
		c.rehearse(ctx)
	case Naming:
		c.awaitName()
	case Lowering:
...
func (c *Coordinator) establishStart(ctx context.Context) {
	...
	// cycle. Learn constructs no subject and matches no structure.
	...
	if !p.Established() {
		...
		// durable, so a demonstration from an unestablished start may learn the goal and no
	...
	c.s.StartReferent = c.showing(res, c.s.StartState, observe.ReferentLearnStart)
}
...
// guards will have to meet later. Reading anything weaker here would let Learn accept a route it
...
func (c *Coordinator) discover(ctx context.Context) {
	...
	// which is what makes "a place Marco cannot recognise cannot be learned" a property of the
	...
	c.s.DestinationReferent = c.showing(res, c.s.DestinationState,
		observe.ReferentLearnDestination)
	...
	//
	// Learn built it here once, and that was the defect: a candidate constructed after the
	// session ended is invisible to the only stage that can raise `AskRehearse`, so Learn
...
//
// Learn arms nothing here and observes nothing here. The runner finds the pending request the
...
func (c *Coordinator) demonstrate(ctx context.Context) {
	...
	if res.Demonstration == nil {
		// The arming did not take. Never silent: a learning flow that quietly watched
...
// Learn only chooses which sentence a person hears.
func (c *Coordinator) evaluate() {
	...
	c.s.Uncertain = nil
	// The VERDICT is the assessment's word and Learn does not overrule it. An ambiguous or
	// invalid demonstration that another example would not fix is refused here rather than
	// carried forward, because the next thing Learn would otherwise say is "want me to try
	...
	// word for a screen — and Learn creates neither. It hands over to the questions that
	// already exist.
...
//
// Learn performs nothing. It does not choose steps, does not retry, does not recover, and cannot
...
//
// The judgement is the ORDINARY one, recomputed from durable memory on every call. Learn sets no
...
// haveTail refuses honestly when the lifecycle behind Learn was never wired.
func (c *Coordinator) haveTail() bool {
	if c.tail != nil {
		return true
	}
	c.refuse(NoTail, "this Director has no learning lifecycle wired behind Learn")
	return false
}
...
//
// A live two-hop learn — Settings Home → Bluetooth & devices → Mouse — produced both candidates
// correctly and rehearsed only one. Learn gets a single exempt rehearsal question, the terminal
...
// demonstrate the first leg again as a separate learn.
//
...
func (c *Coordinator) reviewEdges() {
	...
	//
	// Learn gets one exempt rehearsal question and the terminal leg claims it, so every
