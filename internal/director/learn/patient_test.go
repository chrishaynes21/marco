...
func (w *waitingTail) Rehearse(context.Context) (learn.Attempt, error) {
	i := w.calls
	w.calls++
	if i < len(w.refusals) {
		return learn.Attempt{Attempted: false, Refusal: w.refusals[i]}, nil
	}
...
func patient(t *testing.T, refusals []string, steps int) (learn.Session, *waitingTail) {
	t.Helper()
	inner := newTail()
	inner.granted = true
...
func TestAnUnobservableStartMakesMarcoWaitRatherThanGiveUp(t *testing.T) {
	...
	if s.Phase != learn.WaitingForStart {
		t.Errorf("phase is %q, want %q", s.Phase, learn.WaitingForStart)
	}
...
func TestBeingOnTheWrongRecognisedPlaceWaits(t *testing.T) {
	...
	if s.Phase != learn.WaitingForStart {
		t.Errorf("phase is %q (%s), want %q", s.Phase, s.Refusal, learn.WaitingForStart)
	}
	if w.stubTail.rehearsals != 0 {
...
func TestCancellingWhileWaitingNeverRunsTheRehearsal(t *testing.T) {
	...
	if s := c.Advance(ctx); s.Phase != learn.WaitingForStart {
...
func TestBeingOnAKnownButWrongScreenWaits(t *testing.T) {
	...
	if s.Phase != learn.WaitingForStart {
		t.Errorf("phase is %q (%s), want %q.\nScope is compared before the grant is claimed, "+
			...
			s.Phase, s.Refusal, learn.WaitingForStart)
	}
	if w.stubTail.rehearsals != 0 {
...
func TestAWindowBehindTheForegroundWaits(t *testing.T) {
	...
	if s.Phase != learn.WaitingForStart {
		t.Errorf("phase is %q (%s), want %q.\nNothing was spent — the person simply has not "+
			"brought the window forward yet.", s.Phase, s.Refusal, learn.WaitingForStart)
	}
	if w.stubTail.rehearsals != 0 {
