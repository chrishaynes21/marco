...
// Grounded is what Learn has decided about, and whether it can show you.
//
// # Why this is said at all
//
// Establishing the start is the most consequential thing a learn session does before anybody is
...
func (s Session) Grounded() []GroundedEndpoint {
	...
	if s.Start != "" {
		out = append(out, GroundedEndpoint{
			Label: "START", Role: observe.ReferentLearnStart,
	...
	if s.Route.To != "" {
		out = append(out, GroundedEndpoint{
			Label: "DESTINATION", Role: observe.ReferentLearnDestination,
...
// whatever the person happens to be looking at now. Live, a bare `director learn` status
...
// cannotShow is why a settled endpoint cannot be pointed at, said as a learn-session sentence.
//
...
// about" — and a learn session is not asking anything, so borrowing them verbatim would read as
...
//
// Every line here is read off an object that already owns the fact. Learn adds no number of its
// own, so a Watch panel cannot disagree with the session report.
func (s Session) Watch() []string {
	out := []string{"LEARNING  " + string(s.Phase), "  " + watchStage(s.Phase)}
	if s.Name != "" {
