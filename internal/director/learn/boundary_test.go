...
//
// Learn is the first thing in this system that addresses the user directly and then watches. That
...
//
// It is also what makes the injected-input exclusion safe under Learn. That exclusion lives in
// the platform navigation source, several layers below; Learn cannot reach it to weaken it, and
...
var forbidden = []struct{ fragment, why string }{
	...
	{"internal/platform", "platform adapters, including the input source whose " +
		"injected-input exclusion Learn must not be able to reach"},
	{"internal/winctx", "window activation and focus"},
	{"internal/screen", "screen capture"},
	{"os/exec", "starting processes"},
}

func TestLearnCannotAct(t *testing.T) {
	reachable := map[string]bool{}
	walk("github.com/chaynes-simpleclouds/marco/internal/director/learn", reachable, 0)

	for path := range reachable {
		for _, f := range forbidden {
			if strings.Contains(path, f.fragment) {
				t.Errorf("learn reaches %s\n\treason it is forbidden: %s\n\t"+
					"Learn asks a person to demonstrate; it must never be able to "+
...
// Learn must not be able to write a file either. Its whole durable footprint is one learning
...
func TestLearnCannotOpenAFile(t *testing.T) {
	pkg, err := build.Import(
		"github.com/chaynes-simpleclouds/marco/internal/director/learn", ".", 0)
	if err != nil {
		t.Fatalf("importing the learn package: %v", err)
	}
	for _, imported := range pkg.Imports {
		switch imported {
		case "os", "io/ioutil", "path/filepath", "net", "net/http":
			t.Errorf("learn imports %q; it orchestrates and must persist nothing of its own",
				imported)
		}
	}
}
