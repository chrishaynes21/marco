...
// Showing the user the two decisions Learn makes on their behalf.
//
...
//
// A learn session that cannot point must be IDENTICAL to one that never tried. Not similar:
...
func TestGroundingFailureDoesNotChangeAnythingAboutTheLearnSession(t *testing.T) {
...
func TestEachEndpointIsGroundedOnTheScreenItWasEstablishedOn(t *testing.T) {
	...
	if g.asked[0].role != observe.ReferentLearnStart ||
		g.asked[1].role != observe.ReferentLearnDestination {
...
// A Director that cannot point at anything still learns, and says so rather than going quiet.
func TestADirectorWithNoGroundingStillLearnsAndSaysItCannotShowYou(t *testing.T) {
	s := runToRoute(t, nil)
...
// a learn session is not asking anything. Reusing them verbatim would read as Marco being confused
// about what it is doing.
func TestASettledEndpointThatCannotBePointedAtSaysWhyInLearningWords(t *testing.T) {
	for _, why := range []observe.ReferentUnavailable{
		observe.ReferentNothingWatched,
		observe.ReferentNotOnScreen,
		observe.ReferentNotAPart,
		...
		for _, l := range lines {
			...
			if strings.Contains(l.Say, "asking about") {
				t.Errorf("%s reads %q, which is the wording for a question. Learn is not "+
...
// runToRoute drives one learn session as far as a discovered route, with the given grounding.
//
...
func runToRoute(t *testing.T, g learn.Grounding) learn.Session {
	t.Helper()
	m := newMemory()
