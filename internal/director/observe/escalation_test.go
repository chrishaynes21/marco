...
func TestASufficientReadingBuysNothing(t *testing.T) {
	for _, need := range []observe.Need{observe.NeedWatching, observe.NeedAnswer, observe.NeedToAct} {
		for _, reason := range []observe.SufficiencyReason{
			...
			e := observe.EscalationOf(need, s, canSayWhichState, time.Hour)
...
func TestAFreshIncompleteReadingSettlesFirst(t *testing.T) {
	...
	e := observe.EscalationOf(observe.NeedToAct, s, canSayWhichState, 0)
...
func TestAPersistentlyIncompleteReadingMaySpend(t *testing.T) {
	s := observe.Sufficiency{State: observe.Incomplete,
		Reason: observe.ReasonClientAreaUnpopulated}
	e := observe.EscalationOf(observe.NeedToAct, s, canSayWhichState, 10*time.Second)
...
func TestBackgroundWatchingDoesNotBuyInferenceForAStandingCondition(t *testing.T) {
	...
	for _, waited := range []time.Duration{0, time.Second, time.Minute, time.Hour} {
		e := observe.EscalationOf(observe.NeedWatching, s, canSayWhichState, waited)
		if e.Worth() {
	...
	if !observe.EscalationOf(observe.NeedAnswer, s, canSayWhichState, time.Minute).Worth() {
...
func TestAnUnobservableReadingIsRefusedRatherThanRepaired(t *testing.T) {
	...
	e := observe.EscalationOf(observe.NeedToAct, s, canSayWhichState, time.Hour)
...
// canSayWhichState is a reading that knows which state it is.
//
// Every case in this file predates the semantic dimension and was written about STRUCTURAL
// sufficiency alone. Passing a claimed reading keeps each of them asking exactly what it always
// asked: 37C's rule is that a healthy reading that can say where it is buys nothing, and these
// are the cases that hold it.
