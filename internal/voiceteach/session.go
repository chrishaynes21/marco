...
type Session struct {
	env      Env
	steps    []macroir.Step
	argNames []string // declared args (from the teach phrase's "with name, …" clause)
}

// New starts an empty voice-teach session reading the live OS through env. argNames
