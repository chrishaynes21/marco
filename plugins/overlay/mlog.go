...
// mlog is a minimal inline structured logger for the overlay plugin.
// It mirrors internal/mlog but lives here because plugins are separate modules
// and can't import the engine's internal packages.
//
// Usage: mlogDebug("msg", "key", val) — reads $MARCO_LOG at first call.
...
func mlogWrite(lvl mlogLevel, msg string, kv ...any) {
	...
	s.WriteString(time.Now().Format("[15:04:05]") + "[" + name + "] " + strings.Replace(msg, ": ", " - ", 1))
	for i := 0; i+1 < len(kv); i += 2 {
		s.WriteString(fmt.Sprintf(" %v=%v", kv[i], kv[i+1]))
	}
	fmt.Fprintln(mlogW, s.String())
}
