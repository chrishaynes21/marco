...
// the raw engine error is logged to the HUD alongside the teach offer instead of
...
func TestNoPlayMatchesPrefix(t *testing.T) {
	...
	line := noPlayMatches + `"open the thing" — try: marco teach open the thing`
	if !strings.HasPrefix(line, noPlayMatches) || !strings.Contains(line, "marco teach") {
